...
The `@m!cro` namespace is an uncompromising exercise in minimalist systems design. We write software characterized by engineering at a low level, adhering to three unyielding pillars:
...
* *Simple* is mastering native runtime mechanics to solve that same task deterministically in under 50 lines of code.
...
Every package under the `@m!cro` scope operates under absolute constraints:

- **Zero-Dependency Directive:** A package must have exactly `0` upstream operational dependencies. Supply-chain security is treated as an absolute boundary. If you import a `@m!cro` utility, you know exactly how many lines of code entered your codebase.
